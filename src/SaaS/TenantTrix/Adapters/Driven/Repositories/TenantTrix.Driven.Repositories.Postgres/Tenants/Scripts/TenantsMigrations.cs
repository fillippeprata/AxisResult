...
internal static class TenantsMigrations
{
    private const string MigrationsTable = $"{TenantsDbInit.Schema}.SCHEMA_MIGRATIONS";
    ...
    public static async Task InitializePostgresAsync(string connectionString)
    {
        ...
        await using (var cmd = new NpgsqlCommand(
                         $"CREATE SCHEMA IF NOT EXISTS {TenantsDbInit.Schema};" +
        ...
        foreach (var (version, script) in TenantsDbInit.Migrations)
        {
