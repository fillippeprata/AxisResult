...
internal static class AxisIdentitiesMigrations
{
    private const string MigrationsTable = $"{AxisIdentitiesDbInit.Schema}.SCHEMA_MIGRATIONS";
    ...
    public static async Task InitializePostgresAsync(string connectionString)
    {
        ...
        await using (var cmd = new NpgsqlCommand(
                         $"CREATE SCHEMA IF NOT EXISTS {AxisIdentitiesDbInit.Schema};" +
        ...
        foreach (var (version, script) in AxisIdentitiesDbInit.Migrations)
        {
