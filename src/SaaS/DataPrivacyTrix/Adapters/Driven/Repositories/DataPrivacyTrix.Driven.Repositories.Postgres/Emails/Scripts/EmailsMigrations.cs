...
internal static class EmailsMigrations
{
    private const string MigrationsTable = $"{EmailsDbInit.Schema}.SCHEMA_MIGRATIONS";
    ...
    public static async Task InitializePostgresAsync(string connectionString)
    {
        ...
        await using (var cmd = new NpgsqlCommand(
                         $"CREATE SCHEMA IF NOT EXISTS {EmailsDbInit.Schema};" +
        ...
        foreach (var (version, script) in EmailsDbInit.Migrations)
        {
