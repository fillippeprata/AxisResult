...
public class AxisResultFunctionalValueTaskTests
{
    ...
    [Fact]
    public async Task VT_ThenAsync_Generic_Success_NonGeneric()
    {
    ...
    [Fact]
    public async Task VT_ThenAsync_Generic_Failure_NonGeneric_ReturnsThis()
    {
    ...
    {
        var r = await AxisResult.Error<int>(new[] { E1, AxisError.NotFound("X") }).RecoverNotFoundAsync(() => VTAsync(42));
        Assert.Equal(42, r.Value);
    }
    ...
    {
        var r = await AxisResult.Error<int>(new[] { E1, E2 }).RecoverNotFoundAsync(() => VTAsync(42));
        Assert.True(r.IsFailure);
    }
