...
public class AxisResultFunctionalAsyncTests
{
    ...
    [Fact]
    public async Task ToAxisResultAsync_Generic_Success_Chains()
    {
        var r = await AxisResult.Ok(55).ToAxisResultAsync(value => Task.FromResult(AxisResult.Ok()));
        Assert.True(r.IsSuccess);
    }

    [Fact]
    public async Task ToAxisResultAsync_Generic_Failure_Propagates()
    {
        var r = await AxisResult.Error<int>(E1).ToAxisResultAsync(value => Task.FromResult(AxisResult.Ok()));
        Assert.True(r.IsFailure);
    }
    ...
    public async Task MapErrorAsync_NonGeneric_Failure_Maps()
    {
        var r = await AxisResult.Error(E1).MapErrorAsync(
            _ => Task.FromResult<IEnumerable<AxisError>>([E2]));
    ...
    public async Task MapErrorAsync_NonGeneric_Success_Skips()
    {
        ...
        var r = await src.MapErrorAsync(_ => Task.FromResult<IEnumerable<AxisError>>([E2]));
        Assert.Same(src, r);
    }
    ...
    {
        var r = await AxisResult.Error<int>([E1, AxisError.NotFound("X")]).RecoverNotFoundAsync(() => Task.FromResult(42));
        Assert.Equal(42, r.Value);
    }
    ...
    {
        var r = await AxisResult.Error<int>([E1, E2]).RecoverNotFoundAsync(() => Task.FromResult(42));
        Assert.True(r.IsFailure);
    }
