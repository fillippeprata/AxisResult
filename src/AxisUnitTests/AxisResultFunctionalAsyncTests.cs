...
public class AxisResultFunctionalAsyncTests
{
    ...
    {
        var r = await AxisResult.Ok(55).ToAxisResultAsync(_ => Task.FromResult(AxisResult.Ok()));
        Assert.True(r.IsSuccess);
    }
    ...
    {
        var r = await AxisResult.Error<int>(E1).ToAxisResultAsync(_ => Task.FromResult(AxisResult.Ok()));
        Assert.True(r.IsFailure);
    }
